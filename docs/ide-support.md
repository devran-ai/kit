...
`.claude/commands/` is configured for git tracking via a `.gitignore` negation pattern:
...
This ensures bridge files are available in git worktrees, solving the "Unknown skill" error that occurs when working in worktree-based sessions (Claude Desktop, Antigravity extension).

For other IDE directories (`.cursor/`, `.opencode/`, `.windsurf/`), `kit init` warns if they are gitignored and provides fix instructions.
