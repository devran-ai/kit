...
function runAllChecks(projectRoot) {
  ...
  try {
    ...
    log.warn('Failed to parse manifest.json — aborting verify', { file: manifestPath, error: err.message });
    return buildReport(results);
  }
  ...
  if (fs.existsSync(loadingRulesPath)) {
    try {
      ...
      // --- Check 12b: Cross-reference — alwaysLoadRules files exist ---
  ...
  // --- Check 13: Bridge-sync — command bridges match workflows (all detected IDEs) ---
  const workflowItems = manifest.capabilities?.workflows?.items || [];
  const bridgeDirs = [
    { ide: 'claude', dir: path.join(projectRoot, '.claude', 'commands'), ext: '.md' },
    { ide: 'cursor', dir: path.join(projectRoot, '.cursor', 'commands'), ext: '.md' },
    { ide: 'opencode', dir: path.join(projectRoot, '.opencode', 'commands'), ext: '.md' },
    { ide: 'vscode', dir: path.join(projectRoot, '.github', 'prompts'), ext: '.prompt.md' },
    { ide: 'windsurf', dir: path.join(projectRoot, '.windsurf', 'workflows'), ext: '.md' },
  ];

  for (const { ide, dir, ext } of bridgeDirs) {
    ...
    for (const item of workflowItems) {
      const bridgePath = path.join(dir, `${item.name}${ext}`);
      const bridgeExists = fs.existsSync(bridgePath);
      results.push({
        name: `bridge-sync:${ide}:${item.name}`,
        status: bridgeExists ? 'pass' : 'warn',
        message: bridgeExists
          ? `Bridge exists: ${ide}/${item.name}${ext}`
          : `Missing bridge: ${ide}/${item.name}${ext} (run kit init to generate)`,
      });
    }

    // Check for orphaned bridges
    const bridgeFiles = fs.readdirSync(dir).filter((f) => f.endsWith(ext));
    for (const f of bridgeFiles) {
      const name = path.parse(f).name.replace(/\.prompt$/, '');
      if (!workflowNames.has(name)) {
        results.push({
          name: `bridge-sync:${ide}:orphan:${name}`,
          status: 'warn',
          message: `Orphaned bridge: ${ide}/${f} (no matching workflow)`,
        });
      }
    }
  }
