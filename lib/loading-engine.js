 ...
 * @param {string} workflowName - Name of the workflow
 * @param {object} rules - Pre-loaded rules object
 * @returns {{ agents: string[], skills: string[], bindingType: string, protectedAgents: string[], protectedSkills: string[] }}
 */
function resolveForWorkflowWithRules(workflowName, rules) {
  ...
  if (!match) {
    return { agents: [], skills: [], bindingType: 'none', protectedAgents: [], protectedSkills: [] };
  }
  ...
  return {
    ...
    protectedAgents: match.protectedAgents || [],
    protectedSkills: match.protectedSkills || [],
  };
}
...
function getLoadPlan(taskDescription, workflowName, projectRoot) {
  ...
  const wfResolution = workflowName
    ? resolveForWorkflowWithRules(workflowName, rules)
    : { agents: [], skills: [], protectedAgents: [], protectedSkills: [] };
  ...
  // Step 2.5: Project documentation discovery
  const docConfig = rules.projectDocDiscovery || {};
  let projectDocs = [];
  if (docConfig.enabled !== false && projectRoot) {
    try {
      const { discoverProjectDocs } = require('./doc-discovery');
      const inventory = discoverProjectDocs(projectRoot, {
        maxDocs: docConfig.maxDocs ?? 8,
        maxDepth: docConfig.scanDepth ?? 3,
        maxFiles: docConfig.maxFiles ?? 200,
        domains: taskResolution.matchedDomains,
        domainBoost: docConfig.domainBoost ?? 2,
      });
      projectDocs = inventory.docs;
    } catch (err) {
      const log = require('./logger').createLogger('loading-engine');
      log.warn('Project doc discovery failed — continuing without', { error: err.message });
    }
  }

  // Step 3: Budget enforcement (with protected items from workflow bindings + planning mandates)
  const protectedItems = {
    agents: [...(wfResolution.protectedAgents || [])],
    skills: [
      ...(wfResolution.protectedSkills || []),
      ...(isPlanWorkflow ? (planningMandates.alwaysLoadSkills || []) : []),
    ],
  };
  ...
  return {
    ...
    ...(projectDocs.length > 0 ? { projectDocs } : {}),
