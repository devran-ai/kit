...
function applyUpdate(sourceRoot, targetRoot, dryRun = false) {
  ...
  try {
    ...
    report.warnings.push(`IDE config regeneration failed: ${err.message}`);
    // Non-fatal — IDE configs are optional
  }

  // Regenerate slash command bridges for all detected IDEs
  let detectedIDEs = ['claude'];
  try {
    ...
    if (fs.existsSync(manifestPath)) {
      const manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf-8'));
      const bridges = generateCommandBridges(targetDir, manifest, { projectRoot: targetRoot });
      detectedIDEs = bridges.detectedIDEs;
      const allConfigs = Object.values(bridges).filter(v => v && v.files);
      writeBridgeConfigs(targetRoot, allConfigs, { force: true });
    }
  } catch (err) {
    report.warnings = report.warnings || [];
    report.warnings.push(`Command bridge regeneration failed: ${err.message}`);
    // Non-fatal — command bridges are optional
  }

  // Gitignore pipeline (Steps 1-4) is skipped entirely in shared mode.
  // Shared mode = user ran `kit init --shared` and is intentionally
  // committing `.agent/` with their team. Running narrow/cleanup/add/untrack
  // on a shared-mode project would force `.agent/` into .gitignore and
  // untrack it from the index, silently breaking the team workflow.
  const { isSharedMode } = require('./io');
  const sharedMode = isSharedMode(targetRoot);

  if (sharedMode) {
    report.sharedMode = true;
  } else {
    // Step 1: Narrow blanket .claude/ → .claude/commands/ for CLI discovery
    try {
      const { narrowBlanketClaudeIgnore } = require('./io');
      narrowBlanketClaudeIgnore(targetRoot);
    } catch (err) {
      report.warnings = report.warnings || [];
      report.warnings.push(`Gitignore narrowing failed: ${err.message}`);
    }

    // Step 2: Clean up legacy .claude/* tracking pattern from Kit v5.2.0
    try {
      const { cleanupLegacyClaudeTracking } = require('./io');
      cleanupLegacyClaudeTracking(targetRoot);
    } catch (err) {
      report.warnings = report.warnings || [];
    ...
    // the git index here while keeping the working-tree copies. The
    // untrackKitArtifacts function itself also has a `git check-ignore` gate
  ...
  // Step 5: Regenerate worktree support files
  try {
    const { generateWorktreeInclude, installPostCheckoutHook } = require('./worktree');
    generateWorktreeInclude(targetRoot, detectedIDEs);
    installPostCheckoutHook(targetRoot);
  } catch (err) {
    report.warnings = report.warnings || [];
    report.warnings.push(`Worktree regeneration failed: ${err.message}`);
  }
