 ...
 */
async function pushToTelegram(botToken, commands, scope) {
  ...
  try {
    const response = await fetch(url, {
      ...
      body: JSON.stringify({ commands, scope: { type: resolvedScope } }),
 ...
 * @param {string} [options.scope] - Bot command scope type (default: 'all_private_chats')
...
async function syncBotCommands(projectPath, options = {}) {
  ...
  if (options.dryRun) {
    return { success: true, commands, message: `Dry run: ${commands.length} commands would be synced (scope: ${options.scope || DEFAULT_SCOPE})` };
  }
