 ...
 *   - Pushes commands to all_private_chats scope (same level as plugin, last-write-wins)
 *   - Runs a health check 15s later to detect and fix plugin re-overwrite
...
if (immediate) {
  patchPlugin();
  runGuard().then(({ code, healthCheckDone }) => {
    if (healthCheckDone) {
      // Health check scheduled — wait for it before exiting
      healthCheckDone.then(() => process.exit(code));
    } else {
      process.exit(code);
    }
  });
...
/**
 * Push cached commands to all_private_chats scope.
 *
 * IMPORTANT: Do NOT push to per-chat (chat) scope. In Telegram's hierarchy,
 * chat > all_private_chats > default. A stale per-chat scope with fewer
 * commands will override the correct all_private_chats scope, hiding workflows.
 *
 * @param {string} token - Bot API token
 * @param {Array} commands - Commands array from cache
 * @param {string} label - Log label for context
 * @returns {Promise<boolean>} true if push succeeded
 */
async function pushCommands(token, commands, label) {
  const url = `https://api.telegram.org/bot${token}/setMyCommands`;

  const globalRes = await fetchWithRetry(url, {
    method: 'POST',
  ...
  if (globalData.ok) {
    guardLog('INFO', `[${label}] Menu restored: ${commands.length} commands (all_private_chats)`);
  } else {
    guardLog('WARN', `[${label}] all_private_chats push failed: ${globalData.description || 'unknown'}`);
  }

  return globalData.ok;
}

/**
 * Health check: verify commands on Telegram match cache.
 * If plugin overwrote them, push again.
 * @param {string} token - Bot API token
 * @param {Array} expectedCommands - Expected commands from cache
 * @returns {Promise<void>}
 */
async function healthCheck(token, expectedCommands) {
  ...
  try {
    ...
    if (!data.ok || !Array.isArray(data.result)) {
      guardLog('WARN', '[health-check] Could not read current commands');
      return;
    }

    const currentCount = data.result.length;
    const expectedCount = expectedCommands.length;

    if (currentCount === expectedCount) {
      guardLog('INFO', `[health-check] OK — ${currentCount} commands verified`);
      return;
    }

    // Plugin overwrote our commands — push again
    guardLog('WARN', `[health-check] DRIFT detected: expected ${expectedCount}, found ${currentCount} — re-pushing`);
    await pushCommands(token, expectedCommands, 'retry');
  } catch (err) {
    guardLog('WARN', `[health-check] Error: ${sanitizeError(err.message)}`);
  }
}

/**
 * Execute the guard: wait, read cache, read token, push to Telegram.
 * @returns {Promise<{ code: number, healthCheckDone: Promise<void> | null }>}
 */
async function runGuard() {
  ...
  try {
    ...
    if (!Array.isArray(cache.commands) || cache.commands.length === 0) {
      return { code: 0, healthCheckDone: null }; // Empty cache — nothing to do
    }
    ...
    if (!token) {
      ...
      return { code: 1, healthCheckDone: null };
    }

    // Step 1: Initial push (8s after session start)
    const ok = await pushCommands(token, cache.commands, 'initial');

    // Step 2: Schedule health check — returns a promise the caller can await
    // so the process stays alive until verification completes
    const healthCheckDone = healthCheck(token, cache.commands);

    return { code: ok ? 0 : 1, healthCheckDone };
  } catch (error) {
...
function patchPlugin() {
  ...
  try {
    ...
    for (const ver of versions) {
      ...
      const rel = path.relative(normBase, normTarget);
