 ...
 */
/**
 * Push cached commands to all_private_chats and per-chat scopes.
 * @param {string} token - Bot API token
 * @param {Array} commands - Commands array from cache
 * @param {string[]} users - Allowed user chat IDs
 * @param {string} label - Log label for context
 * @returns {Promise<boolean>} true if all_private_chats push succeeded
 */
async function pushCommands(token, commands, users, label) {
  const url = `https://api.telegram.org/bot${token}/setMyCommands`;

  // Push to all_private_chats scope
  ...
  // Also push to per-chat scopes for maximum specificity
  if (users.length > 0) {
    const results = await Promise.allSettled(
      users.map(async chatId => {
        try {
          ...
          if (!data.ok) {
            guardLog('WARN', `[${label}] Per-chat failed for ${chatId}: ${data.description || 'unknown'}`);
          }
          return data;
        } catch (err) {
          guardLog('WARN', `[${label}] Per-chat error for ${chatId}: ${sanitizeError(err.message)}`);
          throw err;
        }
      })
    );
    const succeeded = results.filter(r => r.status === 'fulfilled' && r.value && r.value.ok).length;
    guardLog('INFO', `[${label}] Per-chat scope: ${succeeded}/${users.length} user(s)`);
  }
 ...
 */
async function healthCheck(token, expectedCommands, users) {
  ...
  try {
    ...
    await pushCommands(token, expectedCommands, users, 'retry');
  } catch (err) {
...
async function runGuard() {
  ...
  try {
    ...
    const ok = await pushCommands(token, cache.commands, users, 'initial');

    // Step 2: Health check after 15s — detect and fix plugin overwrite
    healthCheck(token, cache.commands, users);
    // Fire-and-forget: don't await, let guard exit quickly
