...
async function runGuard() {
  ...
  try {
    ...
    // Strategy: ALWAYS push to all_private_chats first (counters plugin overwrite),
    // then push to per-chat scopes for additional specificity.
    // This ensures private chat menus are restored regardless of plugin timing.

    // Step 1: Push to all_private_chats scope (overrides plugin's default commands)
    ...
    if (globalData.ok) {
      guardLog('INFO', `Menu restored: ${cache.commands.length} commands (all_private_chats scope)`);
    } else {
      guardLog('WARN', `all_private_chats push failed: ${globalData.description || 'unknown'}`);
    }

    // Step 2: Also push to per-chat scopes for maximum specificity
    if (users.length > 0) {
      const results = await Promise.allSettled(
        users.map(async chatId => {
          try {
            ...
            if (!data.ok) {
              guardLog('WARN', `Per-chat failed for user ${chatId}: ${data.description || 'unknown'}`);
            }
            return data;
          } catch (err) {
            guardLog('WARN', `Per-chat error for user ${chatId}: ${sanitizeError(err.message)}`);
            throw err;
          }
        })
      );
      ...
      guardLog('INFO', `Per-chat scope: ${succeeded}/${users.length} user(s) restored`);
    }

    return globalData.ok ? 0 : 1;
  } catch (error) {
