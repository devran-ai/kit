...
const log = createLogger('task-governance');
...
function appendAudit(projectRoot, entry) {
  ...
  if (fs.existsSync(auditPath)) {
    try {
      ...
      log.warn('Corrupted audit log — resetting', { file: auditPath, error: err.message });
      auditLog = { entries: [] };
    }
  }

  const updatedLog = { ...auditLog, entries: [...auditLog.entries, entry] };

  writeJsonAtomic(auditPath, updatedLog);
}
...
function getAuditTrail(projectRoot, taskId) {
  ...
  try {
    ...
    log.warn('Failed to read audit trail', { error: err.message });
    return [];
  }
}
...
function recordDecision(projectRoot, { actor, actorType, action, files, outcome, metadata }) {
  ...
  if (fs.existsSync(auditPath)) {
    try {
      ...
      log.warn('Corrupted audit log in recordDecision — resetting', { file: auditPath, error: err.message });
      auditLog = { entries: [] };
    }
  }

  const withEntry = { ...auditLog, entries: [...auditLog.entries, entry] };

  // Rotate if needed
  const finalLog = rotateIfNeeded(projectRoot, withEntry);

  writeJsonAtomic(auditPath, finalLog);
