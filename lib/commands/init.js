...
function initCommand(options, ctx) {
  ...
  if (!options.shared) {
    steps.push({
      ...
      fn() {
        const { addToGitignore, ensureClaudeCommandsTracked, checkBridgeGitignoreWarnings } = require('../io');
        ...
        // Ensure .claude/commands/ is tracked in git
        try {
          const claudeResult = ensureClaudeCommandsTracked(targetDir);
          if (claudeResult.action === 'updated') {
            log('   ✓ .claude/commands/ configured for git tracking', 'green');
          }
        } catch (err) {
          log(`   ⚠️  Could not configure .claude/commands/ tracking: ${err.message}`, 'yellow');
        }

        // Warn about gitignored IDE bridge directories (S7)
        try {
          const { generateCommandBridges } = require('../command-bridge');
          const manifest = readJsonSafe(path.join(agentPath, 'manifest.json'), {});
          const bridges = generateCommandBridges(agentPath, manifest, { projectRoot: targetDir });
          const gitignoreWarnings = checkBridgeGitignoreWarnings(targetDir, bridges.detectedIDEs);
          for (const w of gitignoreWarnings) log(`   ⚠ ${w}`, 'yellow');
        } catch { /* non-critical */ }
