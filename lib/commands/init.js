  ...
  if (!options.shared) {
    steps.push({
      ...
      fn() {
        ...
        // (e.g. via `git add -A` before gitignore was configured). This
        // guarantees Kit files never pollute a user's tracked tree.
        try {
          const { untrackKitArtifacts } = require('../io');
          const result = untrackKitArtifacts(targetDir);
          if (result.untracked.length > 0) {
            for (const artifact of result.untracked) {
              log(`   ✓ Untracked ${artifact} from git index`, 'green');
            }
            log('   ℹ Commit this change to finalize cleanup', 'cyan');
          }
        } catch (err) {
          log(`   ⚠️  Could not untrack Kit artifacts: ${err.message}`, 'yellow');
        }
      },
    });
  } else {
