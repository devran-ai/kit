...
const log = createLogger('engineering-manager');
...
function getAgentWorkload(projectRoot, agentName) {
  try {
    ...
    log.warn('Failed to count in-progress tasks', { error: err.message });
    return 0;
  }
}
...
function findBestAgent(projectRoot, taskTitle, taskPriority) {
  if (!taskTitle || typeof taskTitle !== 'string') {
    return { agent: 'unassigned', reason: 'Invalid task title' };
  }
