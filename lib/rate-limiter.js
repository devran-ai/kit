...
function createRateLimiter(name, options = {}) {
  ...
  function tryAcquire() {
    ...
    if (state.tokens > 0) {
      state = Object.freeze({ ...state, tokens: state.tokens - 1, totalAllowed: state.totalAllowed + 1 });
      return { allowed: true };
    }

    state = Object.freeze({ ...state, totalRejected: state.totalRejected + 1 });
   ...
   */
  function reset() {
    state = Object.freeze({ ...state, tokens: maxTokens, lastRefillTime: Date.now() });
  }
