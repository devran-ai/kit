...
/** @typedef {'IDLE' | 'ONBOARDING' | 'EXPLORE' | 'PLAN' | 'IMPLEMENT' | 'VERIFY' | 'CHECKPOINT' | 'REVIEW' | 'DEPLOY' | 'MAINTAIN'} WorkflowPhase */
...
/**
 * Builds the next workflow state immutably for a phase transition.
 *
 * @param {object} loadedState - Current workflow state
 * @param {string} currentPhase - Current phase name
 * @param {string} toPhase - Target phase name
 * @param {string} trigger - Trigger description
 * @param {string} timestamp - ISO timestamp
 * @returns {object} New workflow state
 */
function buildTransitionState(loadedState, currentPhase, toPhase, trigger, timestamp) {
  let updatedPhases = { ...loadedState.phases };

  // Mark previous phase as completed
  if (currentPhase !== 'IDLE' && updatedPhases[currentPhase]) {
    updatedPhases = {
      ...updatedPhases,
      [currentPhase]: { ...updatedPhases[currentPhase], completedAt: timestamp, status: 'completed' },
    };
  }
  ...
  if (updatedPhases[toPhase]) {
    updatedPhases = {
      ...updatedPhases,
      [toPhase]: { ...updatedPhases[toPhase], startedAt: timestamp, status: 'active', completedAt: null },
    };
  }
  ...
  return {
    ...
    history: [...existingHistory, { from: currentPhase, to: toPhase, trigger, timestamp }],
  };
}
...
function executeTransition(projectRoot, toPhase, triggerOverride) {
  ...
  if (currentPhase === toPhase) {
    return { success: false, fromPhase: currentPhase, toPhase, trigger: '', guard: '', error: `Already in phase ${toPhase} — no transition needed` };
  }

  const match = findTransition(transitions, currentPhase, toPhase);

  if (!match) {
    const validTargets = transitions.filter((t) => t.from === currentPhase).map((t) => t.to);
    return { success: false, fromPhase: currentPhase, toPhase, trigger: '', guard: '', error: `Invalid transition: ${currentPhase} → ${toPhase}. Valid targets from ${currentPhase}: [${validTargets.join(', ')}]` };
  }

  const timestamp = new Date().toISOString();
  const trigger = triggerOverride || match.trigger;
  const state = buildTransitionState(loadedState, currentPhase, toPhase, trigger, timestamp);
  ...
  try {
    ...
    return { success: false, fromPhase: currentPhase, toPhase, trigger, guard: match.guard, error: `Failed to write state: ${writeError.message}` };
  }

  workflowEvents.emitTransitionComplete(currentPhase, toPhase, trigger);

  return { success: true, fromPhase: currentPhase, toPhase, trigger, guard: match.guard, timestamp };
}
