...
const DOC_DIR_NAMES = Object.freeze([DOCS_DIR, 'doc', 'documentation', '.docs']);
...
const DOC_PATTERNS = Object.freeze([
  ...
  { test: /(?:^|\/)system-design\//i, category: 'architecture', domains: ['architecture'], priority: 0 },
  { test: /(?:^|\/)infra\//i, category: 'architecture', domains: ['architecture', 'devops'], priority: 0 },
  { test: /(?:^|\/)tech-design\//i, category: 'architecture', domains: ['architecture'], priority: 0 },
  { test: /(?:^|\/)design-system\//i, category: 'design-system', domains: ['frontend', 'architecture'], priority: 0 },
  { test: /(?:^|\/)style-guide\//i, category: 'design-system', domains: ['frontend', 'architecture'], priority: 0 },
  { test: /(?:^|\/)ui-kit\//i, category: 'design-system', domains: ['frontend', 'architecture'], priority: 0 },
  { test: /(?:^|\/)(docs?|documentation)\/theme\//i, category: 'design-system', domains: ['frontend'], priority: 0 },
  { test: /(?:^|\/)(docs?|documentation)\/styles\//i, category: 'design-system', domains: ['frontend'], priority: 0 },
  ...
  { test: /(?:^|\/)typography\.md$/i, category: 'design-system', domains: ['frontend'], priority: 0 },
  { test: /(?:^|\/)colors\.md$/i, category: 'design-system', domains: ['frontend'], priority: 0 },
  ...
  // Priority 0 — Onboarding-generated documents
  { test: /(?:^|\/)TECH-STACK-ANALYSIS\.md$/i, category: 'architecture', domains: ['architecture'], priority: 0 },
  { test: /(?:^|\/)COMPETITOR-ANALYSIS\.md$/i, category: 'market-research', domains: ['planning'], priority: 0 },
  { test: /(?:^|\/)PRD\.md$/i, category: 'epic', domains: ['planning'], priority: 0 },
  { test: /(?:^|\/)DB-SCHEMA\.md$/i, category: 'api-spec', domains: ['database', 'backend'], priority: 0 },
  { test: /(?:^|\/)DESIGN-SYSTEM\.md$/i, category: 'design-system', domains: ['frontend', 'architecture'], priority: 0 },
  { test: /(?:^|\/)USER-JOURNEY-MAP\.md$/i, category: 'screen-spec', domains: ['frontend', 'planning'], priority: 0 },
  { test: /(?:^|\/)ONBOARDING-GUIDE\.md$/i, category: 'guide', domains: ['general'], priority: 0 },

  // Priority 1 — High (screens, epics, accessibility, API, guidelines)
  { test: /(?:^|\/)screens\//i, category: 'screen-spec', domains: ['frontend'], priority: 1 },
  { test: /(?:^|\/)(docs?|documentation)\/views\//i, category: 'screen-spec', domains: ['frontend'], priority: 1 },
  { test: /(?:^|\/)(docs?|documentation)\/pages\//i, category: 'screen-spec', domains: ['frontend'], priority: 1 },
  { test: /(?:^|\/)(docs?|documentation)\/layouts\//i, category: 'screen-spec', domains: ['frontend'], priority: 1 },
  { test: /(?:^|\/)wireframes?\//i, category: 'screen-spec', domains: ['frontend'], priority: 1 },
  { test: /(?:^|\/)mockups?\//i, category: 'screen-spec', domains: ['frontend'], priority: 1 },
  ...
  { test: /(?:^|\/)(docs?|documentation)\/features?\//i, category: 'epic', domains: ['planning'], priority: 1 },
  { test: /(?:^|\/)stories\//i, category: 'epic', domains: ['planning'], priority: 1 },
  { test: /(?:^|\/)specs?\//i, category: 'epic', domains: ['planning'], priority: 1 },
  { test: /(?:^|\/)requirements?\//i, category: 'epic', domains: ['planning'], priority: 1 },
  ...
  { test: /(?:^|\/)endpoints?\//i, category: 'api-spec', domains: ['backend'], priority: 1 },
  { test: /(?:^|\/)swagger\//i, category: 'api-spec', domains: ['backend'], priority: 1 },
  { test: /(?:^|\/)graphql\//i, category: 'api-spec', domains: ['backend'], priority: 1 },
  { test: /(?:^|\/)API\.md$/i, category: 'api-spec', domains: ['backend'], priority: 1 },
  { test: /(?:^|\/)openapi/i, category: 'api-spec', domains: ['backend'], priority: 1 },
  { test: /(?:^|\/)guidelines?\//i, category: 'guide', domains: ['general'], priority: 1 },
  { test: /(?:^|\/)standards?\//i, category: 'guide', domains: ['general'], priority: 1 },
  { test: /(?:^|\/)conventions?\//i, category: 'guide', domains: ['general'], priority: 1 },

  // Priority 2 — Medium (guides, roadmap, branding, runbooks)
  { test: /(?:^|\/)guides?\//i, category: 'guide', domains: ['general'], priority: 2 },
  { test: /(?:^|\/)tutorials?\//i, category: 'guide', domains: ['general'], priority: 2 },
  { test: /(?:^|\/)how-to\//i, category: 'guide', domains: ['general'], priority: 2 },
  ...
  { test: /(?:^|\/)playbooks?\//i, category: 'guide', domains: ['devops', 'reliability'], priority: 2 },
  { test: /(?:^|\/)operations?\//i, category: 'guide', domains: ['devops', 'reliability'], priority: 2 },
  { test: /(?:^|\/)sre\//i, category: 'guide', domains: ['devops', 'reliability'], priority: 2 },
...
function scanDocTree(docRoot, projectRoot, maxDepth, maxFiles) {
  // Note: collected uses push (mutation) for performance in recursive walker.
  // This is contained within scanDocTree scope — callers receive the final array.
  const collected = [];
...
function discoverProjectDocs(projectRoot, options = {}) {
  ...
  // Number.isFinite rejects Infinity, -Infinity, NaN (CWE-400 defense)
  const maxDocs = (Number.isFinite(options.maxDocs) && options.maxDocs > 0) ? options.maxDocs : DEFAULT_MAX_DOCS;
  const maxDepth = (Number.isFinite(options.maxDepth) && options.maxDepth > 0) ? options.maxDepth : DEFAULT_MAX_DEPTH;
  const maxFiles = (Number.isFinite(options.maxFiles) && options.maxFiles > 0) ? options.maxFiles : DEFAULT_MAX_FILES;
  const activeDomains = options.domains || [];
  const domainBoost = (Number.isFinite(options.domainBoost) && options.domainBoost >= 0) ? options.domainBoost : DEFAULT_DOMAIN_BOOST;
  ...
  // Canonicalize path — path.resolve always returns absolute, so the caller
  // is trusted (CLI or loading engine). Symlink escape is caught downstream
  // by the relativePath.startsWith('..') guard in scanDocTree.
