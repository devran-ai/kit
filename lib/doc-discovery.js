...
function discoverProjectDocs(projectRoot, options = {}) {
  ...
  const domainBoost = options.domainBoost || DEFAULT_DOMAIN_BOOST;
  ...
  try {
    // Step 1: Find documentation roots
    const docRoots = findDocRoots(projectRoot);
    ...
    for (const root of docRoots) {
      const paths = scanDocTree(root, projectRoot, maxDepth, maxFiles - allPaths.length);
    ...
    if (allPaths.length === 0) {
      return Object.freeze({
        docs: [],
        scannedRoots: docRoots.map((r) => path.relative(projectRoot, r).replace(/\\/g, '/')),
        totalFound: 0,
        returned: 0,
      });
    }
