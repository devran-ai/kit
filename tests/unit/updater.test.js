...
function setupTestDirs() {
  ...
  createFile(TMP_SOURCE, '.agent/rules.md', '# Rules v2');
  ...
  createFile(TMP_TARGET, '.agent/rules.md', '# Rules v1');
...
describe('CLI Updater — Non-Destructive Merge', () => {
  ...
  it('should detect modified files (hash mismatch)', async () => {
    ...
    expect(report.updated).toContain('rules.md');
  ...
  it('should correctly identify preserved files', async () => {
    ...
    expect(updater.isPreservedFile('rules.md')).toBe(false);
  ...
  it('should apply updates when not in dry-run mode', async () => {
    ...
    // Updated file should have new content
    const rulesContent = fs.readFileSync(path.join(TMP_TARGET, '.agent', 'rules.md'), 'utf-8');
    expect(rulesContent).toContain('v2');
